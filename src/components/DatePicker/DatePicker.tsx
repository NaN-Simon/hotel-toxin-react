...
const range = (start: number, end: number) => new Array(end - start).fill(undefined).map((d, i) => i + start);
...
const renderDayContents = (day: number, date: Date) => (
  <span title={date.toString()}>{getDate(date)}</span>
);

function DatePickerCustom({
  name, selectsRange, inline, mask,
}: ISelectsRange) {
